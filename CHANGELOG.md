...
## 2023-09-04 - emadriga
### Changed
- Remove front compiled files (/frontend/srcs/.angular & /frontend/srcs/node_modules)
- Changes to check apis health, should be evaluated on `docker-compose.yml` not `Dockerfile`
- Keep autogenerated NestJS & Angular `.gitignore` on their respective proyect directories, while maintain a general purpose on root directory
